...
export class ServiceNodeWidget extends React.Component {
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new ServiceNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new ServiceNodeModel(node.name, color);
    }
...
export const ServiceNodeWidgetFactory = React.createFactory(ServiceNodeWidget);
