...
export class StatefulSetNodeWidget extends React.Component {
  static defaultProps = {
    node: null,
    color: 'rgb(224, 98, 20)'
  };
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new StatefulSetNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new StatefulSetNodeModel(node.name, color);
    }
...
export const StatefulSetNodeWidgetFactory = React.createFactory(StatefulSetNodeWidget);
