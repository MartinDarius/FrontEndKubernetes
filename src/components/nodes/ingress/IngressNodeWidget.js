...
export class IngressNodeWidget extends React.Component {
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new IngressNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new IngressNodeModel(node.name, color);
    }
...
export const IngressNodeWidgetFactory = React.createFactory(IngressNodeWidget);
